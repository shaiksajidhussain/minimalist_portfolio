  ...
  return (
    <section ref={sectionRef} className="relative bg-[var(--cream)]">
      ...
      {SKILL_GROUPS.map((group, index) => (
        <article
          key={group.title}
          className="ledger-card sticky top-0 min-h-screen flex items-center px-5 sm:px-12 lg:px-20 py-16 sm:py-20 bg-[var(--cream)]"
        ...
        >
          <div className="ledger-panel liquid-card w-full max-w-5xl mx-auto p-7 sm:p-11 min-h-[58vh] flex flex-col origin-center">
            <div className="flex items-baseline justify-between gap-4 mb-10">
              <div>
                <p className="mb-2 font-mono text-[11px] tracking-[0.28em] text-[var(--gold)]">
                  {group.kicker}
                </p>
                <h3 className="font-serif text-4xl sm:text-6xl text-[var(--ink)]">{group.title}</h3>
              </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-4 sm:gap-6 mt-auto">
              {group.items.map((skill) => {
                ...
                return (
                  <div
                    key={skill.name}
                    className="group liquid-card rounded-[22px] px-4 py-5 hover:border-[var(--gold)] transition-colors"
                  >
                    <Icon
                      size={28}
